    ...
    for line in lines {
        ...
        for round in rounds {
            ...
            for selection in selections {
                let mut caps = re.captures_iter(selection);
                let capture = caps.next().unwrap();
        ...
        let mut line_power: usize = 1;

        for key in min_colors.keys() {
            if min_colors.get(key).unwrap() > &0 {
                line_power *= min_colors.get(key).unwrap();
            }
        }
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"#;
    ...
    #[test]
    fn test_full() {
        let input = r#"Game 1: 14 green, 8 blue, 9 red; 5 blue, 4 green, 2 red; 4 red, 4 blue, 4 green; 1 blue, 3 green, 2 red; 10 red, 3 blue, 15 green; 2 red, 6 green, 3 blue
